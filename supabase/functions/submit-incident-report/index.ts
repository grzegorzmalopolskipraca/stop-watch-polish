...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check rate limiting - max 1 incident report per user per type per 5 minutes
    ...
    const { data: recentReports } = await supabase
      ...
      .eq('action_type', `incident_${incidentType}`)
      .gte('last_action_at', fiveMinutesAgo);

    if (recentReports && recentReports.length > 0) {
      return new Response(
        JSON.stringify({ error: 'Możesz zgłosić to zdarzenie ponownie za kilka minut' }),
    ...
    await supabase
      ...
      .upsert({
        ...
        action_type: `incident_${incidentType}`,
        action_count: 1,
